			    ...
			    $("#btnDelete").click(function(){
			        ...
			        $(".check:checked").each(function(index,domEle){
			             ...
			             top.$.dialog.confirm("你确定要注销留言信息?", function(){
				             window.location.href=encodeURI('<%=SystemParam.getSiteRoot()%>/admin/leavewordmanager.do?forwardurl=/admin/leavewordmanager.jsp&actiontype=delete&id='+id);
				         });
			        });
			    });
			...
			<tr>
				...
				<td>
				   <%if(temleaveword.getStatus()==0){ %>
				      <a class="orange-href"
					href="<%=SystemParam.getSiteRoot() %>/admin/lwreply.jsp?id=<%=temleaveword.getId()%>">回复</a>
				   <%} else{%>
					 <a class="orange-href"
					     href="<%=SystemParam.getSiteRoot() %>/admin/leaveworddetails.jsp?id=<%=temleaveword.getId()%>">查看</a>
					<%} %>
				</td>
			</tr>
			<%}}%>
